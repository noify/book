...
vuex 是一种基于vue的状态管理库，集中式存储管理应用的所有组件的状态。

在大型单页应用中，多个组件之间共享状态：不同组件依赖同一个状态；不同组件内的行为会改变该状态。vuex 就是为了解决多组件之间的状态通信问题。

# store

存储状态的容器，包含着应用中大部分的状态 (state)。

- vuex 的状态存储是响应式的
- 只能提交 Mutation 改变 store 中的状态，不可直接改变。
...
可以使用对象展开运算符，将 mapState 函数返回的对象展开。以便与局部计算属性混合使用
...
Getter 可以从 store 中的 state 中派生出一些状态（可以认为是 store 的计算属性）。
...
在 mutation 内，想要对 state 中的对象添加新属性，必须这样：
...
类似于 mutation 的提交（commit），Action 也必须通过 store.dispatch 方法触发：
...
// async / await 必须配合 Promise 食用: 紧跟着 await 之后的必须是 Promise，否则默认状态为 resolved，await 不会等待。await 等待的标准在于 Promise 的状态，Promise 的状态还是pending就一直等待，直到 Promise 的状态发生改变（resolved / reject）
